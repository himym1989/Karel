...
public class BreakoutExt extends WindowProgram {
     ...
     */
    private static final double MAX_Y_VELOCITY = 30;
    ...
    /**
     * Counter of the remaining turns.
     */
    ...
    private void printResultInfo() {
        double[] clip = turns > 0 ? StdAudio.read("assets/sounds/you_win.wav") :
                StdAudio.read("assets/sounds/game_over.wav");
        GLabel resultInfo = new GLabel(
                turns > 0 ? "Congrats! You win the game!" : "Sorry! You lost the game!");
        resultInfo.setFont("Serif-30");
        add(resultInfo, (getWidth() - resultInfo.getWidth()) / 2, (getHeight() + resultInfo.getAscent()) / 2);
        StdAudio.play(clip);
    }
    ...
    /**
     * Method implements creating of the scorebar,
     * where the gamer can track his turns.
     */
